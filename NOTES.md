...
- `api/brain-map/snapshot` OG share image **re-skinned to O&A + now actually renders** (2026-06-28). It had a **pre-existing crash** (predated the redesign — image never generated): satori (`next/og`) requires every multi-child `<div>` to set `display:flex`, and crucially treats `{expr}literal` text (e.g. `{pct}%`, `{examLabel} Brain Map`) as MULTIPLE children. Fixed: added `display:flex` to the header/stat/stat-item divs and collapsed mixed text into single template-literal strings (`{`${pct}%`}`). Also switched `runtime` edge→**nodejs** (verified-rendering; the route does service-role DB queries). Verified: live route returns 200 image/png (50KB) on O&A. Diagnosis tip: satori streaming errors surface only in the dev server log ("Expected <div> to have explicit display:flex…"), not the HTTP response (socket just closes).
