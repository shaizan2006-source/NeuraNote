...
> - **F-036/F-037 (S1, LIVE):** `/api/generate-quiz` (spoofable body `userId`), `/api/generate-document` (fully unauth + CPU-heavy). **Fix in the patch** (route auth + size cap + client call sites).
> - **F-038 (S1, LIVE):** free-tier 20/day Q&A cap **never enforces** — `planLimits.countTodayQA` filters on a non-existent `qa_usage.created_at` column → unlimited free Q&A (cost leak). **Fix in the patch** (`countTodayQA`/`recordQAUsage` use the `(user_id,date)` `count` column). *NB: also edits `planLimits.js`, like the entitlements patch — disjoint hunks; use `git apply --3way` if applying onto a tree that already has the entitlements change.*
...
> - **F-040 (S1, LIVE):** **no rate limiting** on any endpoint — one token can flood every expensive route. **✅ FIXED** — added a sliding-window limiter (`src/lib/rateLimit.js`) wired into the request interception layer. (Correction: the audit said "no `middleware.js`", but Next 16 renamed middleware → **`proxy.js`**, which *does* exist as a no-op; the fix extends it.) → `hotfix-f040-rate-limit-and-build-unblock.patch`.
> - **🛑 F-041 (S0/deploy-blocker, NEW, LIVE):** `origin/main` **does not build** — `src/hooks/useRealtimeProgress.js` imports `@/lib/realtimeDebounce`, which is **absent from git** (added only on the unmerged side-branch `06ca9dfc`; `main` carries the import but never the file). A missing import in a client component is a hard `next build` failure → **every Vercel build of `main` fails → pushes to `main` (incl. the #3 security-fix merge) do NOT deploy; prod is frozen on the last successful build.** Predates this engagement (`a36e914b` has it too). **⚠️ The earlier "fixes are live" conclusion is therefore UNSAFE — verify the Vercel deployment status of the #3 merge.** **Fix:** restore `realtimeDebounce.js` (24-line pure util, recovered from `06ca9dfc`) — build then goes green (verified). Bundled into the F-040 patch.
...
> **Headline:** the team's WIP already fixes F-034/035/036 with auth — but it's stranded undeployed (F-007). The `hotfix-phase4-data-leaks.patch` now closes **F-034–F-038** (both S0 leaks + generate-quiz/generate-document auth + the free-tier cost-cap bug) off `origin/main`, route + client, in **8 files** — verified to apply cleanly. F-039 (pgvector) ships as migration `20260629000003`; F-040 (rate limiting) is architectural (`middleware.js`).
