/**
 * shot-9e.mjs — authenticated capture of the Stage 9 re-skinned pages.
 * Logs in with the dev account (TEST_EMAIL/TEST_PASSWORD from .env.local),
 * then screenshots the finished + re-skinned surfaces at desktop + mobile.
 */
...
async function login(ctx, tag) {
  ...
  // Tolerant: wait to LEAVE /login (any authenticated landing — dashboard/onboarding/etc.)
  try {
    await pg.waitForURL(u => !u.pathname.startsWith("/login"), { timeout: 30000 });
    await pg.waitForTimeout(1500);
    console.log(`  ✓ logged in (${tag}) → ${new URL(pg.url()).pathname}`);
  } catch {
    await pg.screenshot({ path: path.join(OUT, `_login-debug.${tag}.png`) });
    const err = await pg.evaluate(() => document.body.innerText.slice(0, 300)).catch(()=> "");
    console.error(`  ✗ login stuck on /login (${tag}). Page text: ${err.replace(/\n/g, " ")}`);
...
for (const [name, w, h] of [["desktop", 1440, 900], ["mobile", 390, 844]]) {
  ...
  for (const route of ROUTES) {
    ...
    try {
      ...
      console.log(`    ✓ ${route} (${name}) [landed ${new URL(pg.url()).pathname}]`);
      await pg.close();
...
await b.close();
console.log("✅ 9e capture run complete");
